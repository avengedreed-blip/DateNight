...
// Accept an optional `intensity` prop which scales the number of particles
// drawn on the canvas. A value of 1 corresponds to a normal round, 2 to
// spicy, and 3 to extreme. Higher intensities increase particle density.
const ParticleCanvas = memo(({ theme, intensity = 1 }) => {
  ...
  useEffect(() => {
    ...
    const baseCount =
      kind === "hearts"
        ...
        ? 70
        : 50;
    // Apply intensity scaling: increase particle density with higher intensities
    const factor = Math.max(1, intensity || 1);
    const mobileCount = Math.floor(baseCount * 0.6 * factor);
    const desktopCount = Math.floor(baseCount * factor);
